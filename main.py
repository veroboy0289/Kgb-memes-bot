...
async def post_meme():
    ...
    await bot.send_photo(chat_id=CHANNEL_ID, photo=meme_url, caption=caption, reply_markup=reply_markup)

async def main():
    hour, minute = map(int, SCHEDULE_HOUR.split(":"))
    scheduler = AsyncIOScheduler()
    scheduler.add_job(post_meme, "cron", hour=hour, minute=minute)
    scheduler.start()
    await post_meme()
    while True:
